...
def search():
    ...
    if valid and ftype == 'SearchFormFile':
        ...
        if formfile.radius.data == 'value':
            ...
            radius = 500
        return redirect(url_for('get_results', sess_id=session_id,
                                zipcode=user_zip, img=user_file,
                                radius=radius))
        ...
        return redirect(url_for('get_results', sess_id=session_id,
                                zipcode=user_zip, img=user_file,
                                radius=radius, searchtype=searchtype))
    if not valid:
        switch_sect = 'true'
...
def get_results():
    ...
    t = threading.Thread(name='child procs', target=matcher.fit,
                         args=(dog_url, session_id))
    t.start()
