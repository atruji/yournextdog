...
class DoggyMatchEngine(object):
    ...
    def fit(self, user_input, session_id):
        ...
        img = imread(img_file)
        i_size = img.shape
        if i_size[0] < 221 or i_size[1] < 221 or (i_size[2] != 3 and
                                                  i_size[2] != 4):
            raise Exception('Img too small to process.')
        imgr = resize(img, [221, 221])
        self.orig_file = img_file
        img_file = img_file.replace('app/static/uploads/',
                                    'app/static/uploads/r_')
        imsave(img_file, imgr)
        if self.GPU:
            p = subprocess.check_output(["/OverFeat/bin/linux_64/cuda/\
                                        overfeat_cuda", "-f", "-l", img_file])
        else:
            p = subprocess.check_output(["/OverFeat/bin/linux_64/overfeat",
                                        "-f", "-l", img_file])
        features = p.replace('\n', '').replace('1-', '').strip(' ').\
            split(' ')[2:]
    ...
    def sql_import(self, user_zip, user_radius):
        ...
        user_lat = lnglat[0][1]
        self.psql.execute(
            '''select id,features_bin from records where ST_Distance_Sphere
            (geom, ST_MakePoint(%s, %s))*0.000621371 < %s;
    ...
    def match(self, num_matches=10):
        sim_scores = pairwise_distances(np.array(self.user_features),
                                        np.array(self.geo_feature_lst),
                                        'cosine', n_jobs=-1)
        ...
        while dupes:
            ...
            self.top10_scores = np.append(new_top_scores,
                                          sim_scores[repl_match_ind])
    ...
    def results(self):
        conn = boto.connect_s3(os.environ['access_key'],
        ...
        for x, val in enumerate(self.top10_ids):
            self.psql.execute('''select name,gender,age,city,state,
                zip from records where id='%s';''' % val)
            img_details = self.psql.fetchall()[0]
            key = val.encode('ascii', 'ignore') + '.jpg'
            img = bucket.get_key(key)
            outfile = img.generate_url(6000)
            img_data.append([val, outfile, self.top10_scores[x],
                            img_details[0], img_details[1], img_details[2],
                            img_details[3], img_details[4], img_details[5]])
        self.df = pd.DataFrame(np.array(img_data), columns=['id', 'img_loc',
                                                            'score', 'name',
                                                            'gender', 'age',
                                                            'city', 'state',
                                                            'zip'])
